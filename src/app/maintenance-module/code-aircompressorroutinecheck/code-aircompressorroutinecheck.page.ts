...
@Component({
  selector: 'app-code-aircompressorroutinecheck',
  templateUrl: './code-aircompressorroutinecheck.page.html',
  styleUrls: ['./code-aircompressorroutinecheck.page.scss'],
})
export class CodeAircompressorroutinecheckPage implements OnInit {

  userlist = JSON.parse(localStorage.getItem("userlist"));

  aircompressorroutinecheckForm;
  ...
  constructor(public modalController: ModalController, public navParams: NavParams, private fb: FormBuilder, private commonservice: AIREIService, private service: MaintenanceServiceService, private decimalpipe: DecimalPipe) {
    ...
    this.aircompressorroutinecheckForm = this.fb.group({
    ...
    this.service.getdustsettingdetails(req).then((result) => {
      ...
      if (resultdata.httpcode == 200) {
        for (let i = 0; i < resultdata.data.length; i++) {
          ...
          if (getname == "aircompressorroutinecheck") {
  ...
  onChangeMaintenanceType() {
    if (this.aircompressorroutinecheckForm.value.select_maintenancetype != "") {

      this.getWorkOrderNo(JSON.parse(this.aircompressorroutinecheckForm.value.select_maintenancetype).id);
  ...
  onChangeQA1() {
    this.alert_overallcondition(
      "QA1",
      this.aircompressorroutinecheckForm.value.select_qa1
    );
  }
  ...
  onChangeQA2() {
    this.alert_overallcondition(
      "QA2",
      this.aircompressorroutinecheckForm.value.select_qa2
    );
  }
  ...
  onChangeQA3() {
    this.alert_overallcondition(
      "QA3",
      this.aircompressorroutinecheckForm.value.select_qa3
    );
  }
  ...
  onChangeQA4() {
    this.alert_overallcondition(
      "QA4",
      this.aircompressorroutinecheckForm.value.select_qa4
    );
  }
  ...
  onChangeQA5() {
    this.alert_overallcondition(
      "QA5",
      this.aircompressorroutinecheckForm.value.select_qa5
    );
  }
  ...
  save()
  {
    ...
    var getqr_date = moment(this.aircompressorroutinecheckForm.value.txt_date).format("YYYY-MM-DD");

    var getec_date = moment(this.aircompressorroutinecheckForm.value.txt_date).format("YYYY-MM-DD HH:mm:ss");
    ...
    var ans = this.aircompressorroutinecheckForm.value.select_qa1 + "~" 
    + this.aircompressorroutinecheckForm.value.select_qa2 + "~" 
    + this.aircompressorroutinecheckForm.value.select_qa3 + "~"
    + this.aircompressorroutinecheckForm.value.select_qa4 + "~"
    + this.aircompressorroutinecheckForm.value.select_qa5;
    ...
    if (this.aircompressorroutinecheckForm.valid) {
      var req = {
        userid: this.userlist.userId,
        departmentid: this.userlist.dept_id,
        millcode: this.userlist.millcode,
        ...
        type: this.type,
        maintenancetype: String(JSON.parse(this.aircompressorroutinecheckForm.value.select_maintenancetype).id), 
        workordernumber: this.workorderno,
        workordertype: this.aircompressorroutinecheckForm.value.select_worktype,
        urgency: this.aircompressorroutinecheckForm.value.select_urgency,
        mode: this.aircompressorroutinecheckForm.value.select_mode,
        ...
        remarks: this.aircompressorroutinecheckForm.value.ta_remarks,
        fromqrcode: 1,
        qrdate: getqr_date,
        screen: "Air Routine",
        id: 0,
      ...
      this.service.savemachinecheckreport(req).then((result) => {
        ...
        if (resultdata.httpcode == 200) {
          this.aircompressorroutinecheckForm.reset();

          this.commonservice.presentToast(
            "success",
            "Air Compressor Routine Check Inserted Successfully"
          );
          ...
          this.commonservice.presentToast(
            "error",
            "Air Compressor Routine Check Insert Failed"
          );
        }
      });
