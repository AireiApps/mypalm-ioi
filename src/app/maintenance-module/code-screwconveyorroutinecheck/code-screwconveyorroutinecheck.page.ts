...
@Component({
  selector: 'app-code-screwconveyorroutinecheck',
  templateUrl: './code-screwconveyorroutinecheck.page.html',
  styleUrls: ['./code-screwconveyorroutinecheck.page.scss'],
})
export class CodeScrewconveyorroutinecheckPage implements OnInit {

  userlist = JSON.parse(localStorage.getItem("userlist"));

  screwconveyorroutinecheckForm;
  ...
  constructor(public modalController: ModalController, public navParams: NavParams, private fb: FormBuilder, private commonservice: AIREIService, private service: MaintenanceServiceService, private decimalpipe: DecimalPipe) {
    ...
    this.screwconveyorroutinecheckForm = this.fb.group({
    ...
    this.service.getdustsettingdetails(req).then((result) => {
      ...
      if (resultdata.httpcode == 200) {
        for (let i = 0; i < resultdata.data.length; i++) {
          ...
          if (getname == "screwconveyorroutinecheck") {
  ...
  onChangeMaintenanceType() {
    if (this.screwconveyorroutinecheckForm.value.select_maintenancetype != "") {

      this.getWorkOrderNo(JSON.parse(this.screwconveyorroutinecheckForm.value.select_maintenancetype).id);
  ...
  onChangeQA1() {
    this.alert_overallcondition(
      "QA1",
      this.screwconveyorroutinecheckForm.value.select_qa1
    );
  }
  ...
  onChangeQA2() {
    this.alert_overallcondition(
      "QA2",
      this.screwconveyorroutinecheckForm.value.select_qa2
    );
  }
  ...
  onChangeQA3() {
    this.alert_overallcondition(
      "QA3",
      this.screwconveyorroutinecheckForm.value.select_qa3
    );
  }
  ...
  onChangeQA4() {
    this.alert_overallcondition(
      "QA4",
      this.screwconveyorroutinecheckForm.value.select_qa4
    );
  }
  ...
  onChangeQA5() {
    this.alert_overallcondition(
      "QA5",
      this.screwconveyorroutinecheckForm.value.select_qa5
    );
  }
  ...
  save()
  {
    ...
    var getqr_date = moment(this.screwconveyorroutinecheckForm.value.txt_date).format("YYYY-MM-DD");

    var getec_date = moment(this.screwconveyorroutinecheckForm.value.txt_date).format("YYYY-MM-DD HH:mm:ss");
    ...
    var ans = this.screwconveyorroutinecheckForm.value.select_qa1 + "~" 
    + this.screwconveyorroutinecheckForm.value.select_qa2 + "~" 
    + this.screwconveyorroutinecheckForm.value.select_qa3 + "~"
    + this.screwconveyorroutinecheckForm.value.select_qa4 + "~"
    + this.screwconveyorroutinecheckForm.value.select_qa5;
    ...
    if (this.screwconveyorroutinecheckForm.valid) {
      var req = {
        userid: this.userlist.userId,
        departmentid: this.userlist.dept_id,
        millcode: this.userlist.millcode,
        ...
        type: this.type,
        maintenancetype: String(JSON.parse(this.screwconveyorroutinecheckForm.value.select_maintenancetype).id), 
        workordernumber: this.workorderno,
        workordertype: this.screwconveyorroutinecheckForm.value.select_worktype,
        urgency: this.screwconveyorroutinecheckForm.value.select_urgency,
        mode: this.screwconveyorroutinecheckForm.value.select_mode,
        ...
        remarks: this.screwconveyorroutinecheckForm.value.ta_remarks,
        fromqrcode: 1,
        qrdate: getqr_date,
        screen: "Screw Routine",
        id: 0,
      ...
      this.service.savemachinecheckreport(req).then((result) => {
        ...
        if (resultdata.httpcode == 200) {
          this.screwconveyorroutinecheckForm.reset();

          this.commonservice.presentToast(
            "success",
            "Screw Conveyor Routine Check Inserted Successfully"
          );
          ...
          this.commonservice.presentToast(
            "error",
            "Screw Conveyor Routine Check Insert Failed"
          );
        }
      });
