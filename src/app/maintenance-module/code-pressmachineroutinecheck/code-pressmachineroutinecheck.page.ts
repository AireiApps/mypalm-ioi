...
@Component({
  selector: 'app-code-pressmachineroutinecheck',
  templateUrl: './code-pressmachineroutinecheck.page.html',
  styleUrls: ['./code-pressmachineroutinecheck.page.scss'],
})
export class CodePressmachineroutinecheckPage implements OnInit {

  userlist = JSON.parse(localStorage.getItem("userlist"));

  pressmachineroutinecheckForm;
  ...
  constructor(public modalController: ModalController, public navParams: NavParams, private fb: FormBuilder, private commonservice: AIREIService, private service: MaintenanceServiceService, private decimalpipe: DecimalPipe) {
    ...
    this.pressmachineroutinecheckForm = this.fb.group({
    ...
    this.service.getdustsettingdetails(req).then((result) => {
      ...
      if (resultdata.httpcode == 200) {
        for (let i = 0; i < resultdata.data.length; i++) {
          ...
          if (getname == "pressmachineroutinecheck") {
  ...
  onChangeMaintenanceType() {
    if (this.pressmachineroutinecheckForm.value.select_maintenancetype != "") {

      this.getWorkOrderNo(JSON.parse(this.pressmachineroutinecheckForm.value.select_maintenancetype).id);
  ...
  onChangeQA1() {
    this.alert_overallcondition(
      "QA1",
      this.pressmachineroutinecheckForm.value.select_qa1
    );
  }
  ...
  onChangeQA2() {
    this.alert_overallcondition(
      "QA2",
      this.pressmachineroutinecheckForm.value.select_qa2
    );
  }
  ...
  onChangeQA3() {
    this.alert_overallcondition(
      "QA3",
      this.pressmachineroutinecheckForm.value.select_qa3
    );
  }
  ...
  onChangeQA4() {
    this.alert_overallcondition(
      "QA4",
      this.pressmachineroutinecheckForm.value.select_qa4
    );
  }
  ...
  onChangeQA5() {
    this.alert_overallcondition(
      "QA5",
      this.pressmachineroutinecheckForm.value.select_qa5
    );
  }
  ...
  save()
  {
    ...
    var getqr_date = moment(this.pressmachineroutinecheckForm.value.txt_date).format("YYYY-MM-DD");

    var getec_date = moment(this.pressmachineroutinecheckForm.value.txt_date).format("YYYY-MM-DD HH:mm:ss");
    ...
    var ans = this.pressmachineroutinecheckForm.value.select_qa1 + "~" 
    + this.pressmachineroutinecheckForm.value.select_qa2 + "~" 
    + this.pressmachineroutinecheckForm.value.select_qa3 + "~"
    + this.pressmachineroutinecheckForm.value.select_qa4 + "~"
    + this.pressmachineroutinecheckForm.value.select_qa5;
    ...
    if (this.pressmachineroutinecheckForm.valid) {
      var req = {
        userid: this.userlist.userId,
        departmentid: this.userlist.dept_id,
        millcode: this.userlist.millcode,
        ...
        type: this.type,
        maintenancetype: String(JSON.parse(this.pressmachineroutinecheckForm.value.select_maintenancetype).id), 
        workordernumber: this.workorderno,
        workordertype: this.pressmachineroutinecheckForm.value.select_worktype,
        urgency: this.pressmachineroutinecheckForm.value.select_urgency,
        mode: this.pressmachineroutinecheckForm.value.select_mode,
        ...
        remarks: this.pressmachineroutinecheckForm.value.ta_remarks,
        fromqrcode: 1,
        qrdate: getqr_date,
        screen: "Press Routine",
        id: 0,
      ...
      this.service.savemachinecheckreport(req).then((result) => {
        ...
        if (resultdata.httpcode == 200) {
          this.pressmachineroutinecheckForm.reset();

          this.commonservice.presentToast(
            "success",
            "Press Machine Routine Check Inserted Successfully"
          );
          ...
          this.commonservice.presentToast(
            "error",
            "Press Machine Routine Check Insert Failed"
          );
        }
      });
