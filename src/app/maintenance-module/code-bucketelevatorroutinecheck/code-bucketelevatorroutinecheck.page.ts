...
@Component({
  selector: 'app-code-bucketelevatorroutinecheck',
  templateUrl: './code-bucketelevatorroutinecheck.page.html',
  styleUrls: ['./code-bucketelevatorroutinecheck.page.scss'],
})
export class CodeBucketelevatorroutinecheckPage implements OnInit {

  userlist = JSON.parse(localStorage.getItem("userlist"));

  bucketelevatorroutinecheckForm;
  ...
  constructor(public modalController: ModalController, public navParams: NavParams, private fb: FormBuilder, private commonservice: AIREIService, private service: MaintenanceServiceService, private decimalpipe: DecimalPipe) {
    ...
    this.bucketelevatorroutinecheckForm = this.fb.group({
    ...
    this.service.getdustsettingdetails(req).then((result) => {
      ...
      if (resultdata.httpcode == 200) {
        for (let i = 0; i < resultdata.data.length; i++) {
          ...
          if (getname == "bucketelevatorroutinecheck") {
  ...
  onChangeMaintenanceType() {
    if (this.bucketelevatorroutinecheckForm.value.select_maintenancetype != "") {

      this.getWorkOrderNo(JSON.parse(this.bucketelevatorroutinecheckForm.value.select_maintenancetype).id);
  ...
  onChangeQA1() {
    this.alert_overallcondition(
      "QA1",
      this.bucketelevatorroutinecheckForm.value.select_qa1
    );
  }
  ...
  onChangeQA2() {
    this.alert_overallcondition(
      "QA2",
      this.bucketelevatorroutinecheckForm.value.select_qa2
    );
  }
  ...
  onChangeQA3() {
    this.alert_overallcondition(
      "QA3",
      this.bucketelevatorroutinecheckForm.value.select_qa3
    );
  }
  ...
  onChangeQA4() {
    this.alert_overallcondition(
      "QA4",
      this.bucketelevatorroutinecheckForm.value.select_qa4
    );
  }
  ...
  onChangeQA5() {
    this.alert_overallcondition(
      "QA5",
      this.bucketelevatorroutinecheckForm.value.select_qa5
    );
  }
  ...
  save()
  {
    ...
    var getqr_date = moment(this.bucketelevatorroutinecheckForm.value.txt_date).format("YYYY-MM-DD");

    var getec_date = moment(this.bucketelevatorroutinecheckForm.value.txt_date).format("YYYY-MM-DD HH:mm:ss");
    ...
    var ans = this.bucketelevatorroutinecheckForm.value.select_qa1 + "~" 
    + this.bucketelevatorroutinecheckForm.value.select_qa2 + "~" 
    + this.bucketelevatorroutinecheckForm.value.select_qa3 + "~"
    + this.bucketelevatorroutinecheckForm.value.select_qa4 + "~"
    + this.bucketelevatorroutinecheckForm.value.select_qa5;
    ...
    if (this.bucketelevatorroutinecheckForm.valid) {
      var req = {
        userid: this.userlist.userId,
        departmentid: this.userlist.dept_id,
        millcode: this.userlist.millcode,
        ...
        maintenancetype: String(JSON.parse(this.bucketelevatorroutinecheckForm.value.select_maintenancetype).id), 
        workordernumber: this.workorderno,
        workordertype: this.bucketelevatorroutinecheckForm.value.select_worktype,
        urgency: this.bucketelevatorroutinecheckForm.value.select_urgency,
        mode: this.bucketelevatorroutinecheckForm.value.select_mode,
        ...
        remarks: this.bucketelevatorroutinecheckForm.value.ta_remarks,
        fromqrcode: 1,
        qrdate: getqr_date,
        screen: "Bucket Routine",
        id: 0,
      ...
      this.service.savemachinecheckreport(req).then((result) => {
        ...
        if (resultdata.httpcode == 200) {
          this.bucketelevatorroutinecheckForm.reset();

          this.commonservice.presentToast(
            "success",
            "Bucket Routine Check Inserted Successfully"
          );
          ...
          this.commonservice.presentToast(
            "error",
            "Bucket Routine Check Insert Failed"
          );
        }
      });
